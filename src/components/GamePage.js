...
export default function GamePage() {
    ...
    return (
        <div className="min-h-screen flex flex-col items-center p-2 md:p-4">
            <div ref={ref} className="h-full w-full flex flex-col items-center max-w-[40rem] grow">
                ...
                <div className="flex w-full justify-between items-center">
                    <div className="flex">
                        { game ? game.Teams.map(el => <div key={ el } className={ `flex items-center justify-center font-bold cursor-pointer mr-1 w-6 h-6 rounded-full border-4 border-${ el }-500 ${ network && connected && connected[network.Name] === el  ? `bg-${ connected[network.Name] }-500` : "" }` } onClick={ () => setTeam(el) }>{ game && connected && connected[network.Name] !== "" ? game.MoreData.Points[el] : "" }</div>) : null }
                    </div>
                ...
                <div className="w-full flex justify-between items-center">
                    <div className="title leading-4 text-2xl font-black text-indigo-600 cursor-pointer">
