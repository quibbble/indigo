            ...
            <div className="flex flex-col items-center m-8 md:m-12">
                    ...
                    showVariantPopop ? 
                        <div className="absolute w-full h-full top-0 bg-zinc-500 bg-opacity-50 flex items-center justify-center fade-in">
                            <div className="px-4 py-4 bg-zinc-800 mx-2">
                                <p className="font-bold text-center mb-1"><span className="title text-3xl font-black text-indigo-600 mr-1">Indigo</span> variants</p>
                                <p className="text-sm"><span className="font-bold">Classic:</span> standard Indigo.</p>
                                <p className="text-sm"><span className="font-bold">Large Hands:</span> players have a hand size of 2.</p>
                ...
                <div className="w-full max-w-2xl">
                    <div className="flex flex-col items-center fade-in">
                        <div className="title text-5xl font-black text-indigo-600 mb-1 cursor-pointer">
                            <a href={ `${ window.location.protocol }//${ window.location.host }` }>Indigo</a>
                        </div>
                        <div className="font-thin mb-3">
                            Play two to four player Indigo online against friends.
                        ...
                        <div className="flex w-full justify-between flex-wrap">
                            ...
                            <div className="flex items-center order-1 md:order-2 mb-2 md:mb-0">
                                ...
                                <select className="mr-1 bg-zinc-800 text-xs h-6 border font-bold border-zinc-100 focus:outline-none" id="players" onChange={ e => setVariant(e.target.value.replace(/\s/g, "")) }>
                                    { ["Classic", "Large Hands"].map(el => <option key={ el } value={ el }>{ el }</option>) }
                                </select>
                                ...
                                <select className="bg-zinc-800 text-xs h-6 border font-bold border-zinc-100 focus:outline-none" id="players" onChange={ e => setTeams(parseInt(e.target.value)) }>
                                    { [2, 3, 4].map(el => <option key={ el } value={ el }>{ el }</option>) }
                                </select>
                            </div>
                        </div>
                    </div>
